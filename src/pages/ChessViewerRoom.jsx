                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        {roomData?.isOnline && pairing.result === 'pending' && activeRound.status === 'published' && (
                          <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                            <a 
                              href={players.find(p => p.id === pairing.player2)?.lichessId ? `https://lichess.org/?user=${players.find(p => p.id === pairing.player2)?.lichessId}&clock.limit=${(roomData.timeControlLimit || 10)*60}&clock.increment=${roomData.timeControlIncrement || 0}#friend` : '#'} 
                            ...
                            <a 
                              href={players.find(p => p.id === pairing.player1)?.lichessId ? `https://lichess.org/?user=${players.find(p => p.id === pairing.player1)?.lichessId}&clock.limit=${(roomData.timeControlLimit || 10)*60}&clock.increment=${roomData.timeControlIncrement || 0}#friend` : '#'} 
                              target="_blank" rel="noreferrer" onClick={(e) => { if (e.currentTarget.getAttribute('href') === '#') { e.preventDefault(); alert('This player did not provide a Lichess Username during registration.'); } }} 
                              style={{ flex: 1, padding: '10px', textAlign: 'center', background: '#10b981', color: '#000', textDecoration: 'none', borderRadius: '8px', fontSize: '0.85rem', fontWeight: 'bold' }}
                            >
